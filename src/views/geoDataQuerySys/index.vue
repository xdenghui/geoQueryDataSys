<template>
  <div class="dashboard-editor-container">
    <el-row style="background:#fff;padding:16px 16px 0;margin-bottom:32px;" class="chart_box">
      ...
      <div id="main" ref="map" style="width: 100%;height:100vh;"></div>
    </el-row>
    ...
    mapChart() {
      ...
      myMapChart.on("click", function(params) {
        if (params.name in provinces) {
          let proData = require(`../../../static/map/province/${provinces[params.name]}.json`)
          // 如果点击的是34个省、市、自治区，绘制选中地区的二级地图
          console.log(`../../../static/map/province/${provinces[params.name]}.json`, proData);
          _that.$echarts.registerMap(params.name, proData);
          var d = [];
          for (var i = 0; i < proData.features.length; i++) {
            d.push({
              name: proData.features[i].properties.name
            });
          }
          ...
          if (special.indexOf(params.seriesName) >= 0) {
            ...
            // 显示县级地图
            let cityData = require(`../../../static/map/city/${cityMap[params.name]}.json`)
            console.log(`../../../static/map/city/${cityMap[params.name]}.json`, cityData);
            _that.$nextTick(() => {
              _that.$echarts.registerMap(params.name, cityData);
              var d = [];
